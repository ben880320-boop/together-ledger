...
const checks = [
  ...
  ["更新專區具備非 JSON 保護、Wi‑Fi 開關、內容／安全摘要與安全續傳", app.includes('autoDownloadUpdatesOnWifi') && app.includes('Network.getNetworkStateAsync') && app.includes('formatUpdateMessage') && app.includes('更新與下載') && app.includes('僅 Wi‑Fi 自動下載') && app.includes('getUpdateNotesPreview') && app.includes('getUpdateSecuritySummary') && app.includes('readSavedUpdateResume') && app.includes('download.pauseAsync') && app.includes('resumeAndroidUpdate') && readFileSync(resolve(projectRoot, "lib/api.ts"), "utf8").includes('伺服器暫時回傳了非預期內容')],
  ...
  ["Android 交付版本為 1.2.8.6", appConfig.includes('"version": "1.2.8.6"') && appConfig.includes('"versionCode": 16')],
];
