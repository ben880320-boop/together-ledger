...
const mobileApi = readFileSync(resolve(projectRoot, "lib/api.ts"), "utf8");
...
const mobilePackage = readFileSync(resolve(projectRoot, "package.json"), "utf8");
...
const checks = [
  ...
  ["登入／註冊使用電子信箱與密碼並儲存 JWT", app.includes('signInWithFirebaseEmail') && app.includes('registerFirebaseEmailWithProfile') && app.includes('exchangeFirebaseSession') && app.includes('saveSessionToken(result.token)') && router.includes('exchangeFirebaseToken: publicProcedure') && db.includes('createFirebaseUser') && db.includes('verifyLocalPassword')],
  ...
  ["QR Code、分享與跨平台 HTTPS 邀請加入", app.includes('QRCode') && app.includes('Share.share') && app.includes('Linking.addEventListener') && app.includes('inviteCodeFromUrl') && app.includes('https://togetherapp-hdbmsjkf.manus.space/invite?code=${encodeURIComponent(ledger.inviteCode)}') && app.includes('value={inviteLink}')],
  ...
  ["月結算快照提出、第二位成員確認與管理員重新開啟", app.includes('setSettlement(workspace.settlement') && app.includes('setSettlementSnapshot((workspace.settlementSnapshot') && app.includes('api.ledger.settlement.markSettled.mutate') && app.includes('api.ledger.settlement.confirm.mutate') && app.includes('api.ledger.settlement.reopen.mutate') && app.includes('需由另一位成員確認後才會鎖定')],
  ["本月待結算摘要與快照歷史可依月份、狀態篩選", app.includes('本月待結算') && app.includes('api.ledger.settlement.history.query') && app.includes('settlementMonthFilter') && app.includes('settlementStatusFilter') && app.includes('待確認') && app.includes('已鎖定') && app.includes('已重開')],
  ...
  ["通知偏好、提醒日與裝置註冊入口已暫時移除", !app.includes('normalizeNotificationPreferences') && !app.includes('notificationRequestRef') && !app.includes('提醒設定已儲存') && !app.includes('每月提醒日期（1–28 日）') && !app.includes('requestExpoPushToken')],
  ...
  ["個人設定分區、版本資訊與穩定鍵盤避讓", app.includes('SettingsSection') && app.includes('目前版本 v{APP_VERSION}') && app.includes('KeyboardAvoidingView') && app.includes('const usesNativeAndroidResize = Platform.OS === "android"') && app.includes('enabled={enabled ?? !usesNativeAndroidResize}') && app.includes('behavior={Platform.OS === "ios" ? "padding" : undefined}') && !app.includes('android: "height"') && appConfig.includes('"softwareKeyboardLayoutMode": "resize"')],
  ...
  ["個人設定提供密碼、五秒倒數最終確認與安全告別的帳號刪除流程", app.includes('function AccountDeletionModal(') && app.includes('永久刪除帳號') && app.includes('setCountdown(5)') && app.includes('倒數不會自動刪除帳號') && app.includes('帳號已安全刪除。謝謝你曾使用共帳') && app.includes('api.auth.deleteAccount.mutate') && router.includes('deleteAccount: protectedProcedure') && db.includes('deleteUserAccount')],
  ["Firebase Email／Password 帳戶可在近期驗證後自行刪除，非密碼帳戶仍受伺服器保護", router.includes('ctx.user.loginMethod === "firebase-email" && Boolean(ctx.user.firebaseUid)') && router.includes('verifyRecentlyAuthenticatedFirebaseIdentity(input.firebaseIdToken)') && router.includes('只有電子信箱帳密帳號可以在 App 內自行刪除。')],
  ...
  ["Android 交付版本為 v1.3.19／versionCode 42", appConfig.includes('"version": "1.3.19"') && appConfig.includes('"versionCode": 42') && mobilePackage.includes('"version": "1.3.19"') && app.includes('const APP_VERSION = "1.3.19"')],
  ...
  ["帳本設定圖示選擇器不引用未定義常數且具局部錯誤復原", !app.includes('CATEGORY_EMOJI_CHOICES') && !app.includes('PAYMENT_EMOJI_CHOICES') && app.includes('choices={CATEGORY_EMOJI_OPTIONS}') && app.includes('choices={PAYMENT_EMOJI_OPTIONS}') && app.includes('const safeChoices = choices.filter') && app.includes('key={`${emoji}-${index}`}') && app.includes('key={`${item}-${index}`}') && app.includes('SettingsErrorBoundary') && app.includes('android.ledger-settings.render')],
  ["錯誤診斷預設關閉、需明確同意且在後端二次遮蔽", app.includes('const [diagnosticsEnabled, setDiagnosticsEnabled] = useState(false)') && app.includes('api.profile.diagnosticsPreference.query()') && app.includes('api.profile.updateDiagnosticsPreference.mutate({ enabled })') && app.includes('api.profile.reportDiagnostic.mutate') && app.includes('開啟錯誤診斷？') && app.includes('sanitizeDiagnosticStack') && app.includes('android.runtime.unhandled') && router.includes('diagnosticsPreference: protectedProcedure') && router.includes('updateDiagnosticsPreference: protectedProcedure') && router.includes('reportDiagnostic: protectedProcedure') && db.includes('if (!preferences.diagnosticReportsEnabled) return { accepted: false as const };') && db.includes('scrubDiagnosticText') && db.includes('await db.delete(diagnosticReports).where(eq(diagnosticReports.userId, userId));')],
  ["預算門檻提醒與投遞診斷入口已暫時移除", !app.includes('預算 80% 提醒') && !app.includes('預算 100% 提醒') && !app.includes('budgetAlert80Enabled') && !app.includes('budgetAlert100Enabled') && !app.includes('通知狀態') && !app.includes('重新註冊通知') && !app.includes('api.notifications.status.query') && !router.includes('notifyBudgetThresholds({')],
  ["儲蓄桶可於 Android 建立、更新、暫停並查看分配歷程", app.includes('api.ledger.savings.buckets.query') && app.includes('api.ledger.savings.create.mutate') && app.includes('api.ledger.savings.update.mutate') && app.includes('api.ledger.savings.stop.mutate') && app.includes('api.ledger.savings.allocations.query') && app.includes('SavingsBucketModal') && app.includes('SavingsAllocationHistoryModal') && app.includes('每月固定分配金額')],
  ...
  ["儲蓄桶可額外存入並以手動稽核紀錄安全同步", app.includes('api.ledger.savings.addDeposit.mutate') && app.includes('SavingsDepositModal') && app.includes('額外存入') && app.includes('expectedVersion: bucket.version') && app.includes('source === "manual"') && app.includes('plus-circle-outline') && app.includes('存入金額不可超過剩餘目標') && router.includes('addDeposit: protectedProcedure') && db.includes('source: "manual"')],
  ...
  ["Firebase Email/Password 登入、註冊驗證信與非枚舉密碼重設已接入 Android", app.includes('signInWithFirebaseEmail') && app.includes('registerFirebaseEmailWithProfile') && app.includes('requestFirebasePasswordReset') && app.includes('resendFirebaseEmailVerification') && app.includes('驗證信已寄出') && app.includes('忘記密碼') && router.includes('exchangeFirebaseToken: publicProcedure')],
  ["Firebase 已驗證身份可在冷啟動自動換發短效 app session，主動登出會同步清除 Firebase 狀態", app.includes('getPersistedVerifiedFirebaseIdToken') && app.includes('exchangeFirebaseSession') && app.includes('await signOutFromFirebase()') && router.includes('FIREBASE_APP_SESSION_TTL_MS') && router.includes('expiresInMs: FIREBASE_APP_SESSION_TTL_MS')],
  ["記住此裝置採明確同意、冷啟動清除未記住 session，且登出會撤銷偏好", app.includes('記住此裝置') && app.includes('getRememberDevicePreference') && app.includes('await setRememberDevicePreference(false)') && mobileApi.includes('REMEMBER_DEVICE_KEY') && mobileApi.includes('Passwords are never persisted')],
  ["已記住裝置在短效 App session 閒置到期後只會單次以 Firebase 靜默恢復，暫時網路失敗不會誤清除身分", app.includes('rememberedSessionRecoveryPendingRef') && app.includes('restoreRememberedFirebaseSession') && app.includes('authState?.user ?? null') && app.includes('if (await getRememberDevicePreference())') && app.includes('isUnauthorized(loadError)') && app.includes('Network failures must not erase the Firebase refresh token') && app.includes('await exchangeFirebaseSession(firebaseIdToken)')],
  ["既有本機帳戶可在個人設定以相同已驗證信箱綁定 Firebase，且不改變帳本資料", app.includes('Firebase 電子信箱登入') && app.includes('既有帳本資料不會變更') && app.includes('api.auth.firebaseStatus.query') && app.includes('api.auth.linkFirebase.mutate') && app.includes('await saveSessionToken(linked.token)') && app.includes('FirebaseAccountLinkModal') && router.includes('linkFirebase: protectedProcedure')],
  ["Android 提供近期密碼驗證後的新信箱驗證流程，且 token 交換保留記住裝置選擇", app.includes('FirebaseEmailChangeModal') && app.includes('reauthenticateFirebaseEmail') && app.includes('requestFirebaseEmailChangeVerification') && app.includes('修改 Firebase 電子信箱') && app.includes('exchangeFirebaseToken.mutate({ idToken, rememberDevice })') && router.includes('rememberDevice: z.boolean().default(false)')],
];
