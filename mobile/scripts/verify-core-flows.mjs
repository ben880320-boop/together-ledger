...
const checks = [
  ...
  ["GitHub 專案、Release 入口與主題同步新版本提示", app.includes('GITHUB_REPOSITORY_URL') && app.includes('GITHUB_RELEASES_URL') && app.includes('releases/latest') && app.includes('isVersionNewer') && app.includes('發現新版 Together Ledger') && app.includes('confirmText: "前往下載"')],
  ["通知偏好儲存後正規化、同步回饋與競態保護", app.includes('normalizeNotificationPreferences') && app.includes('notificationRequestRef') && app.includes('setSettingsNotice') && app.includes('提醒設定已儲存')],
