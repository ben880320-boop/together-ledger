...
請從 [GitHub Releases](https://github.com/ben880320-boop/together-ledger/releases) 下載最新的 `together-ledger.apk`。目前正式版本為 **v1.3.20**（versionCode 43）。Android 可能會在首次安裝非商店來源的 APK 時要求允許此來源安裝應用程式；請只安裝本專案 Release 頁面所提供的檔案，並可比對同頁的 SHA-256 檢查碼。
...
App 會在登入後檢查 GitHub 最新正式 Release；若偵測到較高版本，會顯示更新內容與安全性摘要、官方來源、下載進度、SHA-256 校驗提示及安裝診斷。個人設定頁也提供 GitHub 專案頁、版本下載與 Wiki 入口。v1.3.20 讓 Android 在伺服器確認登入已撤銷時顯示明確說明、清除記住裝置狀態並要求重新輸入已驗證信箱與密碼；帳本資料不會被刪除。暫時網路錯誤不會清除記住狀態。勾選記住此裝置只保存受保護工作階段、從不保存密碼；請只在個人且有螢幕鎖保護的裝置選取。
