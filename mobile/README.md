...
請從 [GitHub Releases](https://github.com/ben880320-boop/together-ledger/releases) 下載最新的 `together-ledger.apk`。目前正式版本為 **v1.3.18**（versionCode 41）。Android 可能會在首次安裝非商店來源的 APK 時要求允許此來源安裝應用程式；請只安裝本專案 Release 頁面所提供的檔案，並可比對同頁的 SHA-256 檢查碼。
...
App 會在登入後檢查 GitHub 最新正式 Release；若偵測到較高版本，會顯示更新內容與安全性摘要、官方來源、下載進度、SHA-256 校驗提示及安裝診斷。個人設定頁也提供 GitHub 專案頁、版本下載與 Wiki 入口。v1.3.18 在 Firebase Email／Password 帳戶既有的近期密碼驗證之後，再要求五秒倒數的最終確認；倒數不會自動刪帳，成功後會安全清除工作階段並顯示告別訊息。非密碼登入帳戶仍無法以密碼刪除，避免放寬帳戶保護。勾選記住此裝置只保存受保護工作階段、從不保存密碼；請只在個人且有螢幕鎖保護的裝置選取。未勾選時，下次冷啟動需要重新登入；主動登出、帳戶刪除與 session 失效都會撤銷恢復狀態。
