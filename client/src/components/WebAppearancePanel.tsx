...
export function WebAppearancePanel() {
  ...
  return <section className="mt-5 rounded-3xl border border-[var(--border)] bg-[color:var(--card)] p-5 text-[var(--card-foreground)] shadow-sm">
    ...
    {isHistoryOpen && <div className="mt-5 rounded-2xl border border-[var(--border)] bg-[color:var(--popover)] p-4 text-[var(--popover-foreground)]"><div className="flex items-center justify-between gap-3"><div><b>官方更新歷程</b><p className="mt-0.5 text-xs text-[var(--muted-foreground)]">內容直接讀取 GitHub Releases，包含安全性摘要。</p></div><Button type="button" size="sm" variant="ghost" onClick={() => setHistoryOpen(false)}>收合</Button></div>{history === null ? <div className="flex items-center gap-2 py-6 text-sm text-[var(--muted-foreground)]"><Loader2 size={16} className="animate-spin" />正在讀取更新歷程…</div> : historyError ? <p className="py-4 text-sm text-[#E38A98]">{historyError}</p> : <div className="mt-3 max-h-80 divide-y divide-[var(--border)] overflow-y-auto">{history.map((release, index) => { const notes = release.body?.trim() || "包含功能更新、錯誤修正與穩定性改善。"; return <article key={`${release.tag_name}-${index}`} className="py-3"><div className="flex flex-wrap items-center justify-between gap-2"><b>v{release.tag_name?.replace(/^v/i, "") || "未標示版本"}</b><span className="text-xs text-[var(--muted-foreground)]">{release.published_at ? new Date(release.published_at).toLocaleDateString("zh-TW") : "日期未提供"}</span></div><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-[var(--card-foreground)]">{notes.length > 420 ? `${notes.slice(0, 420)}…` : notes}</p><p className="mt-2 flex gap-1.5 text-xs leading-5 text-[#83B696]"><ShieldCheck size={14} className="mt-0.5 shrink-0" />{securitySummary(notes)}</p></article> })}</div>}</div>}
  </section>;
}
