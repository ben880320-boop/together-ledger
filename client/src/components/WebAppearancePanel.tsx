...
import { BookOpen, Loader2, RefreshCw, ShieldCheck, Sparkles } from "lucide-react";
...
const WIKI_URL = "https://github.com/ben880320-boop/together-ledger/wiki";
const SCENES = [
...
export function WebAppearancePanel() {
  ...
  return <section className="mt-5 rounded-3xl border border-[var(--border)] bg-[color:var(--card)] p-5 text-[var(--card-foreground)] shadow-sm">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 text-base font-bold"><Sparkles size={17} />場景主題與更新歷程</h2><p className="mt-1 text-sm text-[var(--muted-foreground)]">此裝置的外觀偏好不會影響共同帳本資料，Android 與網頁可各自選擇喜歡的場景。</p></div><div className="flex flex-wrap gap-2"><a href={WIKI_URL} target="_blank" rel="noreferrer" className="inline-flex h-9 items-center rounded-md border border-[var(--border)] px-3 text-sm font-medium text-[var(--card-foreground)]"><BookOpen size={15} className="mr-1.5" />Wiki 使用說明</a><Button type="button" variant="outline" onClick={() => void openHistory()} className="border-[var(--border)] text-[var(--card-foreground)]"><RefreshCw size={15} className="mr-1.5" />查看更新歷程</Button></div></div>
