...
type RevokeAuditOutcome = "all" | "complete" | "partial" | "failed" | "unknown";
type RevokeAuditRange = "7d" | "30d" | "90d" | "all";
type AdminAccount = {
  id: number;
  name: string | null;
  email: string | null;
  ...
  /** Current memberships other than ledgers this account currently owns. */
  ledgerMembershipCount: number;
  /** Ledgers currently owned by this account. */
  ownedLedgerCount: number;
};

const RECENT_SIGN_IN_MS = 30 * 24 * 60 * 60 * 1_000;
const AUTO_REFRESH_MS = 30_000;

function formatDate(value: Date | string | number | null | undefined, empty = "尚未登入") {
...
function errorMessage(error: unknown) {
  return error instanceof Error ? error.message : "管理操作暫時無法完成，請稍後重試。";
}

function isRecentSignIn(value: Date | string | null) {
  if (!value) return false;
  const time = new Date(value).getTime();
  return Number.isFinite(time) && Date.now() - time <= RECENT_SIGN_IN_MS;
}

function loginMethodLabel(method: string | null) {
...
function verificationLabel(status: AdminAccount["emailVerificationStatus"]) {
  return status === "firebase-verified" ? "Firebase 已驗證" : "舊有資料・未主張";
}

function revokeOutcomeLabel(outcome: RevokeAuditOutcome) {
  if (outcome === "complete") return "完整撤銷";
  if (outcome === "partial") return "部分完成・可重試";
  if (outcome === "failed") return "未開始";
  if (outcome === "unknown") return "舊紀錄・結果未標示";
  return "全部結果";
}

function revokeOutcomeTone(outcome: Exclude<RevokeAuditOutcome, "all">) {
  if (outcome === "complete") return "green" as const;
  if (outcome === "partial") return "amber" as const;
...
export default function AdminConsole() {
  ...
  const [revokeAuditOutcome, setRevokeAuditOutcome] = useState<RevokeAuditOutcome>("all");
  const [revokeAuditRange, setRevokeAuditRange] = useState<RevokeAuditRange>("30d");
  ...
  const revokeAuditInput = useMemo(() => {
    const days = revokeAuditRange === "7d" ? 7 : revokeAuditRange === "30d" ? 30 : revokeAuditRange === "90d" ? 90 : null;
    return {
      limit: 50,
      from: days ? new Date(Date.now() - days * 24 * 60 * 60 * 1_000) : undefined,
      outcome: revokeAuditOutcome,
    };
  }, [revokeAuditOutcome, revokeAuditRange]);

  const summary = trpc.admin.summary.useQuery(undefined, queryOptions);
  const cleanup = trpc.admin.authCleanupStatus.useQuery(undefined, queryOptions);
  const accounts = trpc.admin.listUsers.useQuery({ query }, queryOptions);
  const audits = trpc.admin.audits.useQuery({ limit: 50 }, queryOptions);
  const sessionRevocationAudits = trpc.admin.sessionRevocationAudits.useQuery(revokeAuditInput, queryOptions);
  const operationalOverview = trpc.admin.operationalOverview.useQuery({ days: 7 }, queryOptions);

  const newestUpdate = Math.max(
    summary.dataUpdatedAt,
    cleanup.dataUpdatedAt,
    accounts.dataUpdatedAt,
    audits.dataUpdatedAt,
    sessionRevocationAudits.dataUpdatedAt,
    operationalOverview.dataUpdatedAt,
  );
  ...
  const invalidateAccountData = async () => {
    await Promise.all([
      ...
      utils.admin.sessionRevocationAudits.invalidate(),
  ...
  const refreshAdminData = async () => {
    try {
      await Promise.all([
        ...
        accounts.refetch(),
        audits.refetch(),
        sessionRevocationAudits.refetch(),
        operationalOverview.refetch(),
      ]);
  ...
  const deleteAccount = trpc.admin.deleteUser.useMutation({
    onSuccess: async result => {
      const message = result.firebaseIdentityDeleted
        ? "帳戶已刪除並完成身分清理。"
        : "共帳帳戶已刪除；外部身分清理已記錄供後續處理。";
  ...
  const revokeSessions = trpc.admin.revokeUserSessions.useMutation({
    onSuccess: async result => {
      const message = result.success
        ? result.firebaseSessionsRevoked
          ? "已撤銷 App 與 Firebase 的所有登入。"
          : "已撤銷此帳戶的所有 App 登入。"
  ...
  const dataIsRefreshing = summary.isFetching
    || cleanup.isFetching
    || accounts.isFetching
    || audits.isFetching
    || sessionRevocationAudits.isFetching
    || operationalOverview.isFetching;
  ...
  if (loading) {
    return <main className="min-h-screen bg-[#FFF9F7] p-6"><div className="mx-auto max-w-6xl animate-pulse rounded-3xl bg-white p-8 text-[#9A847B]">正在驗證管理員身分…</div></main>;
  }
  if (!isAdmin) {
    return <main className="min-h-screen bg-[#FFF9F7] p-6"><section className="mx-auto max-w-xl rounded-3xl border border-[#E8D7D1] bg-white p-8 text-center shadow-sm"><ShieldCheck className="mx-auto h-8 w-8 text-[#B56C78]" /><h1 className="mt-4 text-xl font-bold text-[#5F4C47]">沒有管理權限</h1><p className="mt-2 text-sm leading-6 text-[#846E66]">此網頁僅提供具備管理員角色的共帳帳戶使用。</p><Button className="mt-6 bg-[#B56C78]" onClick={() => navigate("/app")}>返回共帳</Button></section></main>;
  }

  return <main className="admin-console min-h-screen bg-[#FFF9F7] px-4 py-6 sm:px-6 lg:px-8">
    <style>{`@media (max-width: 639px) { main.admin-console table { display: block; width: 100%; } main.admin-console table thead { display: none; } main.admin-console table tbody { display: block; } main.admin-console table tr { display: block; margin-bottom: .75rem; border: 1px solid #F0E7E2; border-radius: 1.25rem; padding: .25rem .75rem; } main.admin-console table tr:last-child { margin-bottom: 0; } main.admin-console table td { display: flex; align-items: flex-start; gap: .75rem; padding: .75rem .25rem !important; text-align: left; } main.admin-console table td:first-child { display: block; } main.admin-console table td:nth-child(2)::before { content: "登入方式"; } main.admin-console table td:nth-child(3)::before { content: "驗證狀態"; } main.admin-console table td:nth-child(4)::before { content: "最近登入"; } main.admin-console table td:nth-child(5)::before { content: "安全操作"; } main.admin-console table td:nth-child(n+2)::before { flex: 0 0 4.75rem; color: #8A756B; font-size: .75rem; font-weight: 600; line-height: 1.25rem; } main.admin-console table td:nth-child(5) > div { flex-wrap: wrap; justify-content: flex-start; } }`}</style>
    <div className="mx-auto max-w-6xl space-y-6">
      <header className="flex flex-col gap-4 rounded-3xl bg-[#624B51] p-6 text-white shadow-[0_18px_50px_rgba(98,75,81,0.18)] sm:flex-row sm:items-start sm:justify-between">
        <div><div className="flex items-center gap-2 text-sm font-semibold text-[#F5DADB]"><ShieldCheck className="h-4 w-4" />共帳後台管理</div><h1 className="mt-2 text-2xl font-bold tracking-tight">使用者帳戶管理</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-[#F1D9D8]">可依驗證與登入狀態檢視帳戶，撤銷非管理員帳戶登入，或在嚴格確認後刪除帳戶。此頁不呈現密碼、Firebase UID、帳本名稱、收據或收支明細。</p></div>
        <Button type="button" variant="outline" onClick={() => navigate("/app")} className="border-white/40 bg-white/10 text-white hover:bg-white/20 hover:text-white"><ArrowLeft className="mr-2 h-4 w-4" />返回共帳</Button>
      </header>
...
function Badge({ text, tone }: { text: string; tone: "amber" | "blue" | "green" | "neutral" | "red" }) {
  const color = tone === "amber" ? "bg-[#F5E4D7] text-[#8B5B34]" : tone === "blue" ? "bg-[#EAF1FF] text-[#46608D]" : tone === "green" ? "bg-[#E8F4EA] text-[#42734E]" : tone === "red" ? "bg-[#FFF0F1] text-[#A54856]" : "bg-[#F4F0EE] text-[#6C5B56]";
