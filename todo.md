...
- [ ] 為網頁與 Android App 的帳本首頁加入手動下拉／重新整理控制、最後成功同步時間與不干擾操作的同步狀態
- [ ] 讓新增、編輯與移除收支採用局部快取更新與背景同步，避免整頁重新載入且於失敗時安全回復
- [ ] 在慢速、離線或請求失敗時保留最近可用帳本快照，顯示原因、最後同步時間與可重試操作
- [ ] 在 Android App 的更新檢查失敗時顯示網路原因與重試按鈕，並在首頁顯示可靠的新版本可用提示
- [ ] 加入 Android APK 下載完成後的一鍵安裝引導與權限／安全提示，維持既有續傳與 Wi‑Fi 偏好
- [ ] 同步提升網頁發布版本與 Android app.json、Release Notes、GitHub Actions 的 v1.2.8.9 版本資訊
- [ ] 補齊跨平台同步、快取、更新與下載引導回歸測試，驗證各尺寸網頁、Android 核心流程並發布 v1.2.8.9 APK
