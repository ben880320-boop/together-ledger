...
- [x] 重新逐版稽核 GitHub Release，確保每個版本只顯示對應 tag 的更新內容
- [x] 檢查並補齊 README「專案連結與回報問題」區塊的 Wiki 連結，核對版本與功能說明為最新
- [x] 重新稽核公開 Wiki 各頁的版本、功能與通知停用資訊是否與 v1.3.6 一致
- [x] 將公開 Wiki 版本資訊同步至 v1.3.7／versionCode 30，說明圖示選擇器固定顯示無圖示選項與中文搜尋
- [x] 釐清 Android、Web 與 PWA 帳本設定未顯示新增圖示與「無圖示」選項的根因，修正共享資料、選擇器與發行產物一致性
- [x] 更新 v1.3.7 圖示選擇器回歸測試契約，覆蓋固定顯示「不使用圖示」及展開後中文搜尋的跨端行為
- [x] 將已驗收的圖示選擇器可發現性修復發行為 v1.3.7 固定簽章 APK，提供可覆蓋安裝的 Android 更新
- [x] 評估並記錄 GitHub main 分支保護規則的建議設定，避免未經審閱的直接推送或未通過檢查的合併
- [x] 完成跨端回歸驗收並提供至少六項可執行的產品、效能、安全或可觀測性更新建議

## 分類與支付方式圖示選擇器修正

- [x] 盤點 Android、Web 與 PWA 的分類／支付方式圖示選擇器，釐清其與帳本圖示選擇器不同的資料來源與元件實作
- [x] 在分類與支付方式建立／編輯流程加入可選的「不使用圖示」，儲存後不渲染預設或替代圖示
...
- [x] 重現並定位 Android 個人設定「傳送錯誤診斷」偏好無法儲存的 tRPC、資料庫或用戶端狀態根因
- [x] 修復診斷偏好儲存流程，保留預設關閉、明確同意、可隨時停用與不影響正常使用的隱私界線
- [x] 盤點並優化帳本讀取、同步後的衍生計算與清單更新，減少不必要的重算與重繪
- [x] 強化 Android 與 Web/PWA 的載入狀態、骨架屏與低干擾動畫，支援減少動態效果偏好
- [x] 補齊診斷偏好成功與失敗、資料讀取效能與載入體驗的自動化回歸測試，完成完整驗收與固定簽章 APK 發行

## Web/PWA 首頁資訊架構與響應式改版

- [x] 盤點首頁既有登入、網頁版、Android 下載與 PWA 安裝流程，確保改版不改變現有功能與連結行為
- [x] 重整桌機首頁版面與內容密度，補足產品功能、跨平台使用與安全隱私介紹，避免大螢幕留白過多
- [x] 重整手機首頁的視覺層級、操作按鈕與卡片排版，提升行動瀏覽時的資訊完整性與可讀性
- [x] 將更新歷程收斂為最近發布摘要，並以彈出視窗提供完整歷程，避免首頁過長
- [x] 加入 GitHub 專案與 Wiki 的明確資源入口，並補齊自動化測試、桌機／手機視覺驗證與正式網站發布

## Web/PWA 首頁情境內容與互動優化

- [x] 新增實際使用情境圖文區塊，讓新用戶理解共同記帳、分攤與跨裝置檢視流程
- [x] 在首頁底部新增可存取的 FAQ 區塊，說明跨平台同步、離線快照與隱私邊界
- [x] 為首頁按鈕與介紹卡片加入鍵盤可達的 Hover／Focus 互動與平滑轉場，並尊重減少動態效果偏好
- [x] 補齊首頁內容與互動的自動化測試、桌機／手機視覺驗證及正式網站發布

## 跨平台邀請、待結算與帳本讀取效能

- [x] 盤點既有 Android deep link、Web/PWA 路由、邀請碼驗證、結算資料與帳本工作區讀取流程，確認可沿用的跨端合約與效能瓶頸
...
- [x] 盤點 Android 鍵盤開啟／關閉、輸入欄聚焦與捲動避讓的狀態更新，找出造成畫面反覆上下位移的根因
- [x] 修正 Android 鍵盤關閉後的畫面抖動，同時保留輸入欄自動避讓、關閉鍵盤後的穩定捲動與可重試錯誤回饋
...
- [x] 補齊跨平台提示與忘記密碼流程測試、更新 README／Wiki 交接文件，執行完整驗收與發布流程
...
- [x] 依已確認策略保留既有帳密登入 30 天並顯示強制信箱綁定提醒；未驗證 Firebase 身分改由每日作業清理
- [x] 確認指定帳戶既有 admin 角色，並以後端授權、近期驗證與審計保護管理操作（不宣稱其已 Firebase 綁定）
- [x] 查明「記住此裝置」未生效的跨平台原因，修正 Web／PWA／Android 的 session 恢復與撤銷行為
- [x] 將忘記密碼與重寄驗證改為 Email 輸入彈窗，移除顯眼的既有本機帳密登入按鈕並維持非枚舉安全提示
- [x] 註冊時顯示必須由使用者關閉的未驗證帳戶 24 小時清理提醒，並建立可審計的帳戶清理生命週期
- [x] 建立需近期驗證的 Email 變更流程：驗證目前密碼、輸入新信箱、寄送並確認新信箱後才寫入帳戶
- [x] 建立僅 Web 可見、後端 adminProcedure 強制驗證的使用者管理介面，包含查詢、帳戶狀態、受保護刪除與審計日誌
...
## v1.3.20 撤銷登入可見性與管理員體驗

- [x] 盤點 Web／PWA／Android 的 session 撤銷辨識、登入路由與現有管理員帳戶資料邊界，確認不暴露密碼、token、Firebase UID 或帳本明細
- [x] 實作跨端「登入已被撤銷」狀態提示與安全重新登入入口，保留暫時網路失敗的重試行為
- [x] 優化 Web 管理員撤銷登入互動，加入明確確認、操作中狀態、成功回饋與失敗重試
